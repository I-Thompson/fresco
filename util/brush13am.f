...
*---------------------------------------------------------------------
*     initialize some variables for Fresco
*---------------------------------------------------------------------
      ip1=0; ip2=0; ip3=0; 
...
*------------- FRESCO output so far
	...
	inh = 2    !  longitudinal recoil
      rnl=5. ; hnl=step1
      write(16,83) step1,rmax,rintp,rnl,hnl,npw,absend,inh,b(4),thmax,
     x       	 b(3),iter,energy
...
*---------------------------------------------------------------------
 ...
 884  print*,'            [1] zero-range (fnrng = 0)          '
      ...
      if (ireac.le.4) then ! AMM
      print*,'            [4] Exact finite-range (fresco) '
      endif
      read*,izr
      if(izr.lt.1.or.izr.gt.4) go to 884
      write(18,*) izr
      print*,' >>>> ',izr
      if(izr.eq.1) then
       a(2)=0.0
      ...
      if(ireac.lt.5) then
       ...
       write(18,*) real(a(2))
       print*,' >>>> ',real(a(2))
      else if(izr.eq.4) then ! AMM
       if ((ireac.eq.2).or.(ireac.eq.4)) ip1=0
       else ip1=1
       ip2=-1
      endif
...
!      p(1:3)=    1.000   0.000   1.000 / 
      if (izr.eq.4) then
        ...
        kp=8
        write(16,*) ' '
        write(16,86) kp,0, 1., 0. , 1.
        write(16,89) kp,1, 5, 1., 0.  , 1. ! central
        write(16,89) kp,3, 5, 1., 0.  , 1. ! s-o proj
        write(16,89) kp,4, 5, 1., 0.  , 1. ! s-o target
        write(16,89) kp,7, 5, 1., 0. , 1. ! tensor
        write(16,*) ' '
        endif
      endif
89    format('  &pot kp=',i1,' type=',i1,' shape=',i1,
     x              ' p(1:3) =',3f9.4,' /')
      a(1)=10.41
      a(2)=nodes
      a(3)=nil
...
*------------- FRESCO parameters for PROJ overlap function (AMM)
      if (izr.eq.4) then
        if ((ireac.eq.2).or.(ireac.eq.4)) then
        write(16,500) 
500     format(' &OVERLAP kn1=  10 kn2= 11 ic1=1 ic2=2 in=-1  kind=3 ',
     &  'nn=1 l=0 lmax=2 sn=0.5 j=0.5 kbpot=8 be=2.2260 ipc=0 isc=1 /')
        endif 
      endif
...
!	write(16,92) 2,1,5,D0,FNRNG ! AMM
      kind=5; 
      if (izr.eq.4) then
       d0=0; fnrng=0; kind=7; ip3=0
       if ((ireac.eq.2).or.(ireac.eq.4)) then          
       endif
      endif
	 write(16,92) 2,1,kind,D0,FNRNG,ip1,ip2,ip3

92	format('  &COUPLING icto=',i1,' icfrom=',i1,' kind=',i1,
     x        ' p1=',f9.4,' p2=',f9.4, 
     x        ' ip1=',i3, ' ip2=',i3, ' ip3=',i3, '/')
      if (izr.eq.4) write(16,93) 1,1,1,10,1.0  ! AMoro
      write(16,93) -2,1,1,1,1.0
93	format('  &CFP  in=',i2,' ib=',i1,' ia=',i1,' kn=',i2,
     x        ' a=',f9.4,' /')
